...
const MedicinePage = () => {
    ...
    const getData = useCallback(async () => {
        ...
        setData(newRes.data.data.map((item: any) => {
            item.key = item.id
            delete item.id;
            return item
        }));
    ...
    function deleteConfirm() {
        const names = selectedRowKeys?.map((id: any) => {
            const item: Medicine | undefined = dataSource.find((item: Medicine) => item.key === id);
            return item?.name;
        }) ?? []; // 处理 selectedRowKeys 为 null/undefined 的情况
        const validNames = names.filter(name => name !== undefined);
        const content = validNames.length > 0
            ? validNames.join(', ')
            : '未选中任何药品';
        Modal.confirm({
            title: '确定删除此药品？',
            ...
            <div className={'medicine-table-header'}>
                <div className={'medicine-table-header__left'}>
                    ...
                    <Button disabled={!(selectedRowKeys && selectedRowKeys?.length > 0)}
                            onClick={deleteConfirm}>删除</Button>
                    <Button disabled={selectedRowKeys?.length != 1} onClick={onUpdate}>更新</Button>
                </div>
                <div className={'medicine-table-header__right'}>
                    <Input placeholder={'请输入药品名'} value={searchInfo}
                           onChange={(value) => {
                               setSearchInfo(value)
                           }}
                    />
