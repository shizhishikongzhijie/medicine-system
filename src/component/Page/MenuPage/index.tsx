        ...
        return roles.map((role, roleIndex) => {
            ...
            const generateKeys = (nodes: any[], parentKey = ''): TreeNodeData[] =>
                nodes.map((node, index) => ({
                    ...
                    children: generateKeys(node.children, parentKey ? `${parentKey}-${index}` : `${index}`)
                }));
...
const RoleMenuTabs = ({roleMenus, mainMenus}: { roleMenus?: RoleMenu[], mainMenus?: TreeNodeData[] }) => {
    const [activeKey, setActiveKey] = useState('0');
    return (
        <Tabs activeKey={activeKey} onChange={setActiveKey}>
            {roleMenus && roleMenus.map((item, index) => (
...
const RoleMenuTree = ({roleMenus, mainMenus}: { roleMenus?: RoleMenu, mainMenus?: TreeNodeData[] }) => {
    const [treeData, setTreeData] = useState<TreeNodeData[]>();
    const [value, setValue] = useState<{ initialValue: Value[], newValue: Value[] }>({initialValue: [], newValue: []});
    ...
    return (
        <Tree
            ...
            onSelect={(selectedKey: string, selected: boolean,) => console.log(selectedKey, selected)}
            onChange={(value) => {
                // 确保 value 是数组类型
